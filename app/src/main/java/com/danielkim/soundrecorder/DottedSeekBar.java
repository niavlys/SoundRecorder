...
/**
 * Created by sylvain on 03/01/2017.
 */
    ...
    protected synchronized void onDraw(final Canvas canvas) {
        ...
        if (null != mDotsPositions && 0 != mDotsPositions.length && null != mDotBitmap) {
            ...
            for (int position : mDotsPositions) {
                float rr= (float)position/max;
                float res = rr*width;
                int cpos = (int)res;
                canvas.drawBitmap(mDotBitmap, cpos, 0, null);
                Log.d("ERRERR",Integer.toString(cpos));
            }
        }
        canvas.drawBitmap(mDotBitmap, 0, 0, null);
        canvas.drawBitmap(mDotBitmap, width-30, 0, null);
    }
}